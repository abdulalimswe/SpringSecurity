...
//    private UserEntity owner;
    ...
    public void beforePersist(){
        ...
        //if(userId == null) { throw  new ApiException("Cannot Persist Entity without user ID in requestContext for the thread"); }
        setCreatedAt(now());
        setCreatedBy(userId);
        setUpdatedBy(userId);
        setUpdatedAt(now());
    ...
    public void beforeUpdate(){
        var userId = 0L; //RequestContext.getUserId();
        //if(userId == null) { throw  new ApiException("Cannot Update Entity without user ID in requestContext for the thread"); }
        setUpdatedAt(now());
        setUpdatedBy(userId);
    }
